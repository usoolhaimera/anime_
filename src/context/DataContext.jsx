...
const GetData = ({ children }) => {
  ...
  const [episode, setEpisode] = useState(
    anifyEpisode ? JSON.parse(anifyEpisode) : null
  );
  const [isopen, setisopen] = useState(false);
  const [anifybanner, setanifybanner] = useState("");
  const [animeData, setAnimeData] = useState(
    storeData ? JSON.parse(storeData) : null
  );
  ...
  const fetchNewEpisode = async (newid) => {
    try {
      ...
      const res1 = await axios.get(
        `${import.meta.env.VITE_URL}/meta/anilist/info/${newid}`,
        {
          params: {
            type: "gogoanime",
          },
  ...
  const fetchAnimeEpisode = async (id) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_ANI_URL}episodes/${id}`
      );
  ...
  useEffect(() => {
    try {
      ...
      if (e instanceof DOMException && e.name === "QuotaExceededError") {
        ...
        // Take necessary action, like clearing old data or notifying the user
      }
    }
    try {
    ...
    const anifyImage = async (animeData) => {
      if (animeData && animeData.id) {
        const response = await axios.get(
          `${import.meta.env.VITE_ANI_URL}info/${
            animeData.id
          }?fields=[bannerImage]`
        );
